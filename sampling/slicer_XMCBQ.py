...
def process_folds(ar_mvts_dir_path):
    ...
    xfl_path = join(mvts_folds, samples_path, subfolder, 'X')
    mfl_path = join(mvts_folds, samples_path, subfolder, 'M')
    cfl_path = join(mvts_folds, samples_path, subfolder, 'C')
    bfl_path = join(mvts_folds, samples_path, subfolder, 'B')
    flq_path = join(mvts_folds, samples_path, subfolder, 'Q')
    ...
    print((xfl_path, mfl_path, cfl_path, bfl_path, flq_path))
    paths = {'X': xfl_path,
             'M': mfl_path,
             'C': cfl_path,
             'B': bfl_path,
    ...
    try:
        ...
        if (not isdir(xfl_path)) or (not isdir(mfl_path)) or (not isdir(cfl_path)) or (not isdir(bfl_path)) or (not isdir(flq_path)):
            raise
    ...
    for ar_mvts in ar_mvts_files:
        ...
        try:
            ...
            create_slices(mvts_df, non_matched_fl_df, ar_no, paths, filtered_path, use_clear_window=USE_CLEAR_WINDOW)
            ar_count += 1
...
def create_slices(df, shadowdf, arno, paths, filtered_path, use_clear_window=False):
    ...
    for i in range(0, df.shape[0] - (OBS_LENGTH+LATENCY), STEP_FOR_SAMPLING):
        ...
        if pred_window[['MFLARE', 'XFLARE']].sum().sum() == 0:  # if there are no M or X class flares
            ...
            # save the C-class, B-class or non-flaring (flare quiet) instance
            ...
            if max_BC.startswith('B'): # then there is a B flare
                observation.to_csv('{0}/{1}.csv'.format(paths['B'], filename), sep='\t')
            elif max_BC.startswith('C'): # then there is C flare
                observation.to_csv('{0}/{1}.csv'.format(paths['C'], filename), sep='\t')
            elif max_BC.startswith('FQ'): # then there is no flare
                observation.to_csv('{0}/{1}.csv'.format(paths['Q'], filename), sep='\t')
            ...
            if len(verified_MX) > 0:  #then we know there is a verified flare here, create a valid instance
                ...
                if max_target.startswith('X'):  # then there is a B flare
                    observation.to_csv('{0}/{1}.csv'.format(paths['X'], filename), sep='\t')
                elif max_target.startswith('M'):  # then there is C flare
                    observation.to_csv('{0}/{1}.csv'.format(paths['M'], filename), sep='\t')
